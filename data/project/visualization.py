...
def number_of_entries(dataset: RentalDataset) -> None:
    properties = ["Person.name", "Address.country"]
    values = [
        [person.name for person in dataset.people],
        [address.country for address in dataset.addresses]
    ]
...
def addresses_by_country(dataset: RentalDataset) -> None:
    cimek = list({address.country for address in dataset.addresses})
    values = [0 for _ in cimek]
    for address in dataset.addresses:
        values[cimek.index(address.country)] += 1

    x = np.arange(len(cimek))  # the label locations
    width = 0.35  # the width of the bars
    ...
    ax.set_title("Number of countries by addresses")
    ax.set_xticks(x)
    ax.set_xticklabels(cimek, rotation=90)
    ax.bar_label(series)
...
def distances_by_types(dataset: RentalDataset) -> None:
    ...
    ax.pie(values, labels=cimek, autopct="%1.1f%%", startangle=90, rotatelabels=True, pctdistance=0.7)
